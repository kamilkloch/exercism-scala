...
class ChangeTest extends FunSuite with Matchers {
  ...
  test("50k coin change (stack safe)") {
    Change.findFewestCoins(50000, List(1)) should be(Some(List.fill(50000)(1)))
  }

  test("stack overflow @2k") {
    def foo(n: Int): Unit = if (n <= 0) () else {foo(n - 1); ()}

    intercept[java.lang.StackOverflowError](foo(2000))
  }
}